...
where $l_{sat.}$ is the 'saturated model' such that each value of $\mu_i = y_i$ and $l_{model}$ is the  likelihood of $y_i$ for the predicted value of $\mu_i$.  We will not go into the relationship between $\mu_i$, the co-variates $X$, and the fit parameters $B$ here. 
...
# Poisson deviance
abs(y - w * mu) * sqrt(2 * (dpois(y, y, log = T) - dpois(y, w * mu, log = TRUE)))

# Binomial deviance
abs(y - w * mu) * sqrt(2 * (dbinom(y, w, y / w, log = T) - dbinom(y, w, mu, log = TRUE)))
```
