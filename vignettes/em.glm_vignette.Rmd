...
The majority of the heavy lifting of the package relies on the  'em.glm' function.  The function requires a matrix of co-variates (*x*), a target vector (*y*), a GLM family (defaulting to *poisson()*) and the number of target classes (*K*).  Generally taking the form:
...
``` {r, echo = F, fig.height = 7, fig.width = 8, fig.cap = "Residual diagnostic plots for Poisson fit"}
{
  par(mfrow = c(2, 1))
  plot(
    log(y),
...
pois.em <- em.glm(x = x, y = y, K = 2, b.init = "random", param_errors = F)
...
Here we fit a second simulated data set, `{r, eval = F} sim.1` - similar to the previous behavior but with only a single underling Poisson process.  We first establish the true model:
...
``` {r, echo = F, fig.cap = "Single parameter set EM fit"}
